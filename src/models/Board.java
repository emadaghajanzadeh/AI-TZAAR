...
public class Board implements Comparable<Board> , Serializable {
    ...
    public int compareTo(Board o) {
        ...
        for (int i = 0; i < totalRowsCount; i++) {
            for (int j = 0; j < rows[i].size - 1; j++) {
                if(this.rows[i].boardCells[j]!=o.rows[i].boardCells[j]){
                    isEqual = 1;
                    continue;
                }
            }
    ...
    public static class BoardRow implements Serializable {
    ...
    public static class BoardCell implements Serializable {
...
//        clear center of board
    ...
    public String[] getBoardElement(){
        ...
        for (int i = 0; i < totalRowsCount; i++) {
            for (int j = 0; j < rows[i].size - 1; j++) {
                    temp[count++] = this.rows[i].boardCells[j].bead.toString();
            }
            temp[count++]="n";
        }
        return temp;
